"""
US macro heatmap — two-block model.
...
Block 2 — UST curve (daily, separate pipeline):
  Daily UST yields → PCA → daily PC scores.
  PC1/PC2 regressed on Block 1 daily factors (no intercept).
  10y UST fair value reconstructed via PCA inversion.
...
Series metadata is read from data/series_catalogue_us.json via
macro_data_loader._load_dfm_meta(m_macro=56, catalogue_path=...).
...
_US_CATALOGUE_PATH = Path(__file__).parent / "data" / "series_catalogue_us.json"

# ---------------------------------------------------------------------------
# DFM series metadata — single source of truth: series_catalogue_us.json
# ---------------------------------------------------------------------------

_dfm_meta = _load_dfm_meta(m_macro=56, catalogue_path=_US_CATALOGUE_PATH)
...
M_MACRO_US = len(_MACRO_SERIES_IDS)  # 56

UST_TENORS  = ["2y", "5y", "10y", "20y", "30y"]
N_TENORS    = len(UST_TENORS)        # 5
US_10Y_IDX  = UST_TENORS.index("10y")   # 2
...
# Global Macro sign anchor: S&P Global Composite PMI covers both mfg and services.
# Sign convention: Global Macro ↑ = better overall conditions → Composite PMI ↑.
_GM_PRIMARY_COL  = next(e["dfm_col_index"] for e in _dfm_meta if e["id"] == "us_spglobal_composite_pmi")
_GM_PRIMARY_SIGN = 1
...
# ---------------------------------------------------------------------------
# Block 2 yield parameters — UST curve
# ---------------------------------------------------------------------------

# Approximate UST yield means (%, approx Jun 2025)
UST_YIELD_MEANS = np.array([4.00, 4.15, 4.48, 4.85, 4.75])

# How each macro factor drives each UST tenor (simulation only).
# Columns: GlobalMacro, Growth, Inflation, Employment, Wages
# Short end: Fed policy / inflation driven; long end: growth / term premium.
_US_YIELD_FACTOR_LOADINGS = np.array([
    # GlobalMacro  Growth  Inflation  Employment  Wages
    [   0.10,       0.12,    0.50,      0.08,     0.10],  # 2y  (Fed policy / inflation)
    [   0.12,       0.14,    0.40,      0.06,     0.08],  # 5y
    [   0.13,       0.18,    0.28,      0.06,     0.08],  # 10y
    [   0.14,       0.22,    0.20,      0.05,     0.06],  # 20y
    [   0.15,       0.24,    0.16,      0.05,     0.06],  # 30y (term premium / growth)
])
...
def _simulate_macro_block(rng: np.random.Generator) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Simulate K=5 daily factors and M=56 US macro observations (simulation fallback).

    Factor structure mirrors the real model:
      col 0  (Global Macro) : loads on all series
      col 1  (Growth)       : loads on Growth group (cols 0-25)
      col 2  (Inflation)    : loads on Inflation group (cols 44-55)
      col 3  (Employment)   : loads on Employment group (cols 26-37)
      col 4  (Wages)        : loads on Wages group (cols 38-43)

    Returns
    -------
    factors    : [T_DAILY, K]
    Lambda_sim : [M_MACRO_US, K]
    Y_monthly  : [T_MONTHLY, M_MACRO_US]  observations at month-end
    """
    ...
    Lambda_sim = np.zeros((M_MACRO_US, K))
    ...
    noise_std = np.full(M_MACRO_US, 0.3)
    Y_monthly = (
        factors[MONTH_END_IDX] @ Lambda_sim.T
        + rng.standard_normal((T_MONTHLY, M_MACRO_US)) * noise_std
    )
    return factors, Lambda_sim, Y_monthly


def _build_macro_dfm_sim(
    Lambda_sim: np.ndarray,    # [M_MACRO_US, K]
    Y_monthly:  np.ndarray,    # [T_MONTHLY, M_MACRO_US]
    ...
    Y_daily_sim = np.full((T_DAILY, M_MACRO_US), np.nan)
...
def _simulate_yield_block(
    ...
    """
    Simulate daily UST yields from latent factors.
    Returns [T_DAILY, N_TENORS].
    """
    noise = rng.standard_normal((T_DAILY, N_TENORS)) * 0.025
    return UST_YIELD_MEANS[None, :] + true_factors @ _US_YIELD_FACTOR_LOADINGS.T + noise
...
def _compute_pca(
    ...
    for k, pivot_col in enumerate([US_10Y_IDX, N_TENORS - 1, 1]):
...
def _block_pca_1(Y_std: np.ndarray, cols: list[int], primary_col: int, primary_sign: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    loadings : [M_MACRO_US] sparse loading vector (zeros outside `cols`)
    """
    T, M = Y_std.shape
...
def _build_macro_dfm_from_data(
    Y_daily: np.ndarray,  # [T_DAILY, M_MACRO_US] — NaN except on (estimated) release dates
) -> tuple[np.ndarray, np.ndarray]:
    """
    Block-PCA initialisation + Kalman smoother for the 5-factor US model.

    Factor structure
    ----------------
    Factor 0 — Global Macro : first PC of ALL 56 series (sign: S&P Global Composite PMI ↑)
    Factor 1 — Growth       : first PC of Growth group  (cols 0-25, sign: ISM Mfg PMI ↑)
    Factor 2 — Inflation    : first PC of Inflation group (cols 44-55, sign: CPI Core ↑)
    Factor 3 — Employment   : first PC of Employment group (cols 26-37, sign: Unemployment Rate ↓)
    Factor 4 — Wages        : first PC of Wages group   (cols 38-43, sign: AHE y/y ↑)

    Loading matrix Λ [56, 5] is block-structured:
      - Each named factor (cols 1-4) has non-zero loadings only for its group.
      - Global Macro (col 0) has non-zero loadings for all 56 series.
    ...
    Pipeline
    --------
    1.  Forward-fill Y_daily (LOCF) → Y_ffill [T, 56], no NaNs.
    2.  Standardise each column using statistics from actual observations only.
    3.  Within-group PCA for each named factor → score [T] + sparse loading [56].
    4.  Full-panel PCA for Global Macro → score [T] + dense loading [56].
    5.  Assemble F_init [T, 5] and Λ_std [56, 5].
...
def _fetch_macro_data_us(start: str, end: str) -> dict[str, pd.Series] | None:
    """
    Attempt to pull US macro series from Bloomberg or Haver.
    ...
    kwargs["catalogue_path"] = _US_CATALOGUE_PATH
    ...
    try:
        ...
        if not data:
            warnings.warn(
                "[us_heatmap] Fetcher returned no data; falling back to simulation.",
                stacklevel=1,
            )
            return None
        return data
        ...
        warnings.warn(
            f"[us_heatmap] Data fetcher failed ({exc}); falling back to simulation.",
            stacklevel=1,
        )
        return None
...
def _fetch_yield_data_us(start: str, end: str) -> "np.ndarray | None":
    """
    ...
    kwargs: dict = {"catalogue_path": _US_CATALOGUE_PATH}
    if source == "haver":
        haver_path = os.environ.get("HAVER_PATH")
        if haver_path:
            kwargs["path"] = haver_path
    ...
    try:
        fetcher = get_fetcher(source, **kwargs)   # type: ignore[arg-type]
        data = fetcher.fetch(_YIELD_SERIES_IDS_US, start=start, end=end)
        if not data:
            warnings.warn("[us_heatmap] No yield data from fetcher; simulating yields.", stacklevel=1)
            return None

        daily_arr = np.array(DAILY_DATES, dtype="datetime64[D]")
        yield_mat = np.full((T_DAILY, N_TENORS), np.nan)

        for k, sid in enumerate(_YIELD_SERIES_IDS_US):
            series = data.get(sid)
        ...
        if np.isnan(yield_mat).any():
            warnings.warn("[us_heatmap] Yield matrix still has NaNs after fill; simulating.", stacklevel=1)
            return None

        return yield_mat

    except Exception as exc:
        warnings.warn(f"[us_heatmap] Yield fetch failed ({exc}); simulating yields.", stacklevel=1)
        return None
...
_live_data   = _fetch_macro_data_us(_fetch_start, _fetch_end)

_macro_data: MacroData = load_macro_data(
    daily_dates=DAILY_DATES,
    m_macro=M_MACRO_US,
    data=_live_data,   # None → uses simulation
    catalogue_path=_US_CATALOGUE_PATH,
)

for _w in _macro_data.warnings:
    warnings.warn(f"[us_heatmap] {_w}", stacklevel=1)
...
# ── Block 2: yield PCA ────────────────────────────────────────────────────────
_live_yields = _fetch_yield_data_us(_fetch_start, _fetch_end)

if _live_yields is not None:
    UST_YIELDS = _live_yields
else:
    _sim_src = _true_factors if not _sufficient else FACTORS_SMOOTH
    UST_YIELDS = _simulate_yield_block(_sim_src, _rng)

PC_SCORES, PC_LOADINGS, PC_EXPLAINED_VAR, YIELD_MEANS = _compute_pca(UST_YIELDS)
...
# Fair value for 10y UST
UST_PCA_FITTED = (
    YIELD_MEANS[US_10Y_IDX]
    + PC_SCORES[:, 0] * PC_LOADINGS[0, US_10Y_IDX]
    + PC_SCORES[:, 1] * PC_LOADINGS[1, US_10Y_IDX]
)
UST_MACRO_FV = (
    YIELD_MEANS[US_10Y_IDX]
    + PC1_REG["fitted"] * PC_LOADINGS[0, US_10Y_IDX]
    + PC2_REG["fitted"] * PC_LOADINGS[1, US_10Y_IDX]
)
UST_RICH_CHEAP_BPS = (UST_YIELDS[:, US_10Y_IDX] - UST_MACRO_FV) * 100
...
def get_us_daily_factors() -> DailyFactorsResponse:
...
def get_us_fair_value() -> FairValueResponse:
    dates = [d.strftime("%Y-%m-%d") for d in DAILY_DATES]
    return FairValueResponse(
        dates=dates,
        actual=UST_YIELDS[:, US_10Y_IDX].round(4).tolist(),
        pca_fitted=UST_PCA_FITTED.round(4).tolist(),
        macro_fair_value=UST_MACRO_FV.round(4).tolist(),
        rich_cheap_bps=UST_RICH_CHEAP_BPS.round(2).tolist(),
    )


def get_us_yield_pca() -> YieldPCAResponse:
    dates = [d.strftime("%Y-%m-%d") for d in DAILY_DATES]
    ...
    return YieldPCAResponse(
        ...
        tenor_names=UST_TENORS,
    )


def get_us_pc_regressions() -> PCRegressionResponse:
