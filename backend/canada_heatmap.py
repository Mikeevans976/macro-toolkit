"""
Canada macro heatmap — two-block model.
...
Block 2 — CanGov curve (daily, separate pipeline):
  Daily CanGov yields → PCA → daily PC scores.
  PC1/PC2 regressed on Block 1 daily factors (no intercept).
  10y CanGov fair value reconstructed via PCA inversion.
...
Series metadata is read from data/series_catalogue_ca.json via
macro_data_loader._load_dfm_meta(m_macro=43, catalogue_path=...).
...
# ---------------------------------------------------------------------------
# Canada catalogue path
# ---------------------------------------------------------------------------

_CA_CATALOGUE_PATH = Path(__file__).parent / "data" / "series_catalogue_ca.json"

# ---------------------------------------------------------------------------
# DFM series metadata — single source of truth: series_catalogue_ca.json
# ---------------------------------------------------------------------------

_dfm_meta = _load_dfm_meta(m_macro=43, catalogue_path=_CA_CATALOGUE_PATH)
...
M_MACRO_CA  = len(_MACRO_SERIES_IDS)  # 43

CANGOV_TENORS = ["2y", "5y", "10y", "20y", "30y"]
N_TENORS      = len(CANGOV_TENORS)        # 5
CA_10Y_IDX    = CANGOV_TENORS.index("10y")   # 2
...
# Global Macro sign anchor: S&P Global Composite PMI.
# Sign convention: Global Macro ↑ = better overall conditions → Composite PMI ↑.
_GM_PRIMARY_COL  = next(e["dfm_col_index"] for e in _dfm_meta if e["id"] == "ca_spglobal_composite_pmi")
_GM_PRIMARY_SIGN = 1
...
# ---------------------------------------------------------------------------
# Block 2 yield parameters — CanGov curve
# ---------------------------------------------------------------------------

# Approximate CanGov yield means (%, approx Jun 2025)
CANGOV_YIELD_MEANS = np.array([3.10, 3.20, 3.40, 3.65, 3.60])

# How each macro factor drives each CanGov tenor (simulation only).
# Columns: GlobalMacro, Growth, Inflation, Employment, Wages
_CA_YIELD_FACTOR_LOADINGS = np.array([
...
def _simulate_macro_block(rng: np.random.Generator) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Simulate K=5 daily factors and M=43 Canada macro observations (simulation fallback).

    Returns
    -------
    factors    : [T_DAILY, K]
    Lambda_sim : [M_MACRO_CA, K]
    Y_monthly  : [T_MONTHLY, M_MACRO_CA]  observations at month-end
    """
    ...
    Lambda_sim = np.zeros((M_MACRO_CA, K))
    ...
    noise_std = np.full(M_MACRO_CA, 0.3)
    Y_monthly = (
        factors[MONTH_END_IDX] @ Lambda_sim.T
        + rng.standard_normal((T_MONTHLY, M_MACRO_CA)) * noise_std
    )
    return factors, Lambda_sim, Y_monthly


def _build_macro_dfm_sim(
    Lambda_sim: np.ndarray,    # [M_MACRO_CA, K]
    Y_monthly:  np.ndarray,    # [T_MONTHLY, M_MACRO_CA]
    ...
    Y_daily_sim = np.full((T_DAILY, M_MACRO_CA), np.nan)
...
# ---------------------------------------------------------------------------
# Block 2 — CanGov yield simulation
# ---------------------------------------------------------------------------
...
def _simulate_yield_block(
    ...
    """
    Simulate daily CanGov yields from latent factors.
    Returns [T_DAILY, N_TENORS].
    """
    noise = rng.standard_normal((T_DAILY, N_TENORS)) * 0.020
    return CANGOV_YIELD_MEANS[None, :] + true_factors @ _CA_YIELD_FACTOR_LOADINGS.T + noise
...
def _compute_pca(
    ...
    Sign conventions for CanGov PCA:
    ...
    for k, pivot_col in enumerate([CA_10Y_IDX, N_TENORS - 1, 1]):
...
def _block_pca_1(Y_std: np.ndarray, cols: list[int], primary_col: int, primary_sign: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    loadings : [M_MACRO_CA] sparse loading vector (zeros outside `cols`)
    """
    T, M = Y_std.shape
...
def _build_macro_dfm_from_data(
    Y_daily: np.ndarray,  # [T_DAILY, M_MACRO_CA] — NaN except on (estimated) release dates
) -> tuple[np.ndarray, np.ndarray]:
    """
    Block-PCA initialisation + Kalman smoother for the 5-factor Canada model.

    Factor structure
    ----------------
    Factor 0 — Global Macro : first PC of ALL 43 series (sign: S&P Global Composite PMI ↑)
...
def _fetch_macro_data_ca(start: str, end: str) -> dict[str, pd.Series] | None:
    """
    Attempt to pull Canada macro series from Bloomberg or Haver.
    ...
    kwargs["catalogue_path"] = _CA_CATALOGUE_PATH
    ...
    try:
        ...
        if not data:
            warnings.warn(
                "[canada_heatmap] Fetcher returned no data; falling back to simulation.",
                stacklevel=1,
            )
            return None
        return data
        ...
        warnings.warn(
            f"[canada_heatmap] Data fetcher failed ({exc}); falling back to simulation.",
            stacklevel=1,
        )
        return None
...
def _fetch_yield_data_ca(start: str, end: str) -> "np.ndarray | None":
    """
    ...
    kwargs: dict = {"catalogue_path": _CA_CATALOGUE_PATH}
    if source == "haver":
        haver_path = os.environ.get("HAVER_PATH")
        if haver_path:
            kwargs["path"] = haver_path
    ...
    try:
        fetcher = get_fetcher(source, **kwargs)   # type: ignore[arg-type]
        data = fetcher.fetch(_YIELD_SERIES_IDS_CA, start=start, end=end)
        if not data:
            warnings.warn("[ca_heatmap] No yield data from fetcher; simulating yields.", stacklevel=1)
            return None

        daily_arr = np.array(DAILY_DATES, dtype="datetime64[D]")
        yield_mat = np.full((T_DAILY, N_TENORS), np.nan)

        for k, sid in enumerate(_YIELD_SERIES_IDS_CA):
            series = data.get(sid)
        ...
        if np.isnan(yield_mat).any():
            warnings.warn("[ca_heatmap] Yield matrix still has NaNs after fill; simulating.", stacklevel=1)
            return None

        return yield_mat

    except Exception as exc:
        warnings.warn(f"[ca_heatmap] Yield fetch failed ({exc}); simulating yields.", stacklevel=1)
        return None
...
_live_data   = _fetch_macro_data_ca(_fetch_start, _fetch_end)

_macro_data: MacroData = load_macro_data(
    daily_dates=DAILY_DATES,
    m_macro=M_MACRO_CA,
    data=_live_data,   # None → uses simulation
    catalogue_path=_CA_CATALOGUE_PATH,
)

for _w in _macro_data.warnings:
    warnings.warn(f"[canada_heatmap] {_w}", stacklevel=1)
...
# ── Block 2: yield PCA ────────────────────────────────────────────────────────
_live_yields = _fetch_yield_data_ca(_fetch_start, _fetch_end)

if _live_yields is not None:
    CANGOV_YIELDS = _live_yields
else:
    _sim_src = _true_factors if not _sufficient else FACTORS_SMOOTH
    CANGOV_YIELDS = _simulate_yield_block(_sim_src, _rng)

PC_SCORES, PC_LOADINGS, PC_EXPLAINED_VAR, YIELD_MEANS = _compute_pca(CANGOV_YIELDS)
...
# Fair value for 10y CanGov
CANGOV_PCA_FITTED = (
    YIELD_MEANS[CA_10Y_IDX]
    + PC_SCORES[:, 0] * PC_LOADINGS[0, CA_10Y_IDX]
    + PC_SCORES[:, 1] * PC_LOADINGS[1, CA_10Y_IDX]
)
CANGOV_MACRO_FV = (
    YIELD_MEANS[CA_10Y_IDX]
    + PC1_REG["fitted"] * PC_LOADINGS[0, CA_10Y_IDX]
    + PC2_REG["fitted"] * PC_LOADINGS[1, CA_10Y_IDX]
)
CANGOV_RICH_CHEAP_BPS = (CANGOV_YIELDS[:, CA_10Y_IDX] - CANGOV_MACRO_FV) * 100
...
def get_ca_daily_factors() -> DailyFactorsResponse:
...
def get_ca_fair_value() -> FairValueResponse:
    dates = [d.strftime("%Y-%m-%d") for d in DAILY_DATES]
    return FairValueResponse(
        dates=dates,
        actual=CANGOV_YIELDS[:, CA_10Y_IDX].round(4).tolist(),
        pca_fitted=CANGOV_PCA_FITTED.round(4).tolist(),
        macro_fair_value=CANGOV_MACRO_FV.round(4).tolist(),
        rich_cheap_bps=CANGOV_RICH_CHEAP_BPS.round(2).tolist(),
    )


def get_ca_yield_pca() -> YieldPCAResponse:
    dates = [d.strftime("%Y-%m-%d") for d in DAILY_DATES]
    ...
    return YieldPCAResponse(
        ...
        tenor_names=CANGOV_TENORS,
    )


def get_ca_pc_regressions() -> PCRegressionResponse:
