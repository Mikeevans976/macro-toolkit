"""
Australia macro heatmap — two-block model.
...
Block 2 — ACGB curve (daily, separate pipeline):
  Daily ACGB yields → PCA → daily PC scores.
  PC1/PC2 regressed on Block 1 daily factors (no intercept).
  10y ACGB fair value reconstructed via PCA inversion.
...
Series metadata is read from data/series_catalogue_au.json via
...
# ---------------------------------------------------------------------------
# Australia catalogue path
# ---------------------------------------------------------------------------

_AU_CATALOGUE_PATH = Path(__file__).parent / "data" / "series_catalogue_au.json"

# ---------------------------------------------------------------------------
# DFM series metadata — single source of truth: series_catalogue_au.json
# ---------------------------------------------------------------------------

_dfm_meta = _load_dfm_meta(m_macro=42, catalogue_path=_AU_CATALOGUE_PATH)
...
M_MACRO_AU  = len(_MACRO_SERIES_IDS)  # 42

ACGB_TENORS = ["2y", "5y", "10y", "20y", "30y"]
N_TENORS    = len(ACGB_TENORS)        # 5
AU_10Y_IDX  = ACGB_TENORS.index("10y")   # 2
...
_GM_PRIMARY_COL  = next(e["dfm_col_index"] for e in _dfm_meta if e["id"] == "au_pmi_manufacturing")
_GM_PRIMARY_SIGN = 1
...
# ---------------------------------------------------------------------------
# Block 2 yield parameters — ACGB curve
# ---------------------------------------------------------------------------

# Approximate ACGB yield means (%, approx Jun 2025)
ACGB_YIELD_MEANS = np.array([3.80, 4.00, 4.35, 4.75, 4.80])

# How each macro factor drives each ACGB tenor (simulation only).
# Columns: GlobalMacro, Growth, Inflation, Employment, Wages
_AU_YIELD_FACTOR_LOADINGS = np.array([
...
def _simulate_macro_block(rng: np.random.Generator) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Simulate K=5 daily factors and M=42 Australia macro observations (simulation fallback).

    Returns
    -------
    factors    : [T_DAILY, K]
    Lambda_sim : [M_MACRO_AU, K]
    Y_monthly  : [T_MONTHLY, M_MACRO_AU]  observations at month-end
    """
    ...
    Lambda_sim = np.zeros((M_MACRO_AU, K))
    ...
    noise_std = np.full(M_MACRO_AU, 0.3)
    Y_monthly = (
        factors[MONTH_END_IDX] @ Lambda_sim.T
        + rng.standard_normal((T_MONTHLY, M_MACRO_AU)) * noise_std
    )
    return factors, Lambda_sim, Y_monthly


def _build_macro_dfm_sim(
    Lambda_sim: np.ndarray,    # [M_MACRO_AU, K]
    Y_monthly:  np.ndarray,    # [T_MONTHLY, M_MACRO_AU]
    ...
    Y_daily_sim = np.full((T_DAILY, M_MACRO_AU), np.nan)
...
def _simulate_yield_block(
    ...
    """
    Simulate daily ACGB yields from latent factors.
    Returns [T_DAILY, N_TENORS].
    """
    noise = rng.standard_normal((T_DAILY, N_TENORS)) * 0.022
    return ACGB_YIELD_MEANS[None, :] + true_factors @ _AU_YIELD_FACTOR_LOADINGS.T + noise
...
def _compute_pca(
    ...
    for k, pivot_col in enumerate([AU_10Y_IDX, N_TENORS - 1, 1]):
...
def _block_pca_1(Y_std: np.ndarray, cols: list[int], primary_col: int, primary_sign: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    loadings : [M_MACRO_AU] sparse loading vector (zeros outside `cols`)
    """
    T, M = Y_std.shape
...
def _build_macro_dfm_from_data(
    Y_daily: np.ndarray,  # [T_DAILY, M_MACRO_AU] — NaN except on (estimated) release dates
) -> tuple[np.ndarray, np.ndarray]:
    """
    Block-PCA initialisation + Kalman smoother for the 5-factor Australia model.
...
def _fetch_macro_data_au(start: str, end: str) -> dict[str, pd.Series] | None:
    """
    Attempt to pull Australia macro series from Bloomberg or Haver.
    ...
    kwargs["catalogue_path"] = _AU_CATALOGUE_PATH
    ...
    try:
        ...
        if not data:
            warnings.warn(
                "[australia_heatmap] Fetcher returned no data; falling back to simulation.",
                stacklevel=1,
            )
            return None
        return data
        ...
        warnings.warn(
            f"[australia_heatmap] Data fetcher failed ({exc}); falling back to simulation.",
            stacklevel=1,
        )
        return None
...
_live_data   = _fetch_macro_data_au(_fetch_start, _fetch_end)

_macro_data: MacroData = load_macro_data(
    daily_dates=DAILY_DATES,
    m_macro=M_MACRO_AU,
    data=_live_data,   # None → uses simulation
    catalogue_path=_AU_CATALOGUE_PATH,
)

for _w in _macro_data.warnings:
    warnings.warn(f"[australia_heatmap] {_w}", stacklevel=1)
...
# Fair value for 10y ACGB
ACGB_PCA_FITTED = (
    YIELD_MEANS[AU_10Y_IDX]
    + PC_SCORES[:, 0] * PC_LOADINGS[0, AU_10Y_IDX]
    + PC_SCORES[:, 1] * PC_LOADINGS[1, AU_10Y_IDX]
)
ACGB_MACRO_FV = (
    YIELD_MEANS[AU_10Y_IDX]
    + PC1_REG["fitted"] * PC_LOADINGS[0, AU_10Y_IDX]
    + PC2_REG["fitted"] * PC_LOADINGS[1, AU_10Y_IDX]
)
ACGB_RICH_CHEAP_BPS = (ACGB_YIELDS[:, AU_10Y_IDX] - ACGB_MACRO_FV) * 100
...
def get_au_daily_factors() -> DailyFactorsResponse:
...
def get_au_fair_value() -> FairValueResponse:
    dates = [d.strftime("%Y-%m-%d") for d in DAILY_DATES]
    return FairValueResponse(
        dates=dates,
        actual=ACGB_YIELDS[:, AU_10Y_IDX].round(4).tolist(),
        pca_fitted=ACGB_PCA_FITTED.round(4).tolist(),
        macro_fair_value=ACGB_MACRO_FV.round(4).tolist(),
        rich_cheap_bps=ACGB_RICH_CHEAP_BPS.round(2).tolist(),
    )


def get_au_yield_pca() -> YieldPCAResponse:
    dates = [d.strftime("%Y-%m-%d") for d in DAILY_DATES]
    ...
    return YieldPCAResponse(
        ...
        tenor_names=ACGB_TENORS,
    )


def get_au_pc_regressions() -> PCRegressionResponse:
